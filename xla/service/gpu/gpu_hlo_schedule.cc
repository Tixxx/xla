...
enum class GpuResourceType {
  ...
  kGpuAsyncStreamComputes = 3,  // The resource for collective operations.
...
class GpuAsyncTrackerBase : public AsyncTracker {
 public:
  ...
  bool IsAsynComputeOp(const HloInstruction& hlo) const {
    return (hlo.opcode() == HloOpcode::kAsyncStart || hlo.opcode() == HloOpcode::kAsyncDone) &&
...
class GpuAsyncTracker : public GpuAsyncTrackerBase {
 public:
  ...
  int64_t GetNumAvailableResources(int64_t resource_type) const override {
    ...
    if((resource_type - first_target_resource) == 3) {
      return 2;
    }
    return 1;
  }
