...
absl::Status NvshmemCollectiveThunk::Initialize(const InitializeParams& params) {
  if (async_events_) {
...
absl::Status NvshmemCollectiveThunk::ExecuteOnStream(const ExecuteParams& params) {
  ...
  if (IsAsync()) {
    ...
    TF_RETURN_IF_ERROR(RunNvshmemCollective(params, *params.stream, comm_handle));
  }
