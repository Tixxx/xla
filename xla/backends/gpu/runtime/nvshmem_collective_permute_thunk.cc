...
absl::Status NvshmemCollectivePermuteStartThunk::RunNvshmemCollective(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      ConvertToDeviceBuffers(
          params, std::vector<NvshmemCollectiveThunk::Buffer>(buffers_),
  ...
  return ::xla::gpu::RunCollectivePermute(collectives, source_target,
                                          device_buffers, stream, device_string,
                                          current_id);
}
...
absl::Status RunCollectivePermute(
  ...
  for (uint64_t idx = 0; idx < buffers.size(); ++idx) {
    ...
    if (target_id) {
      ...
      TF_RETURN_IF_ERROR(xla::gpu::NvshmemCollectives::Default()->DoPut(
          NvshmemCollectives::TEAMSKIND::kNODE, buffer.element_type, dest_addr,
          src_addr, buffer.element_count, *target_id, stream));
    }
    ...
    if (source_id) {
      ...
      TF_RETURN_IF_ERROR(xla::gpu::NvshmemCollectives::Default()->DoTeamBarrier(
          NvshmemCollectives::TEAMSKIND::kNODE, stream));
    }
  }
...
/*static*/ absl::Status NvshmemCollectivePermuteStartThunk::CheckImplementable(
  ...
  if (NcclCollectivePermuteStartThunk::IsDegenerate(inst, replica_count,
                                                    partition_count)) {
    return absl::OkStatus();
  }
...
absl::Status NvshmemCollectivePermuteDoneThunk::ExecuteOnStream(
  ...
  return xla::gpu::NvshmemCollectives::Default()->DoFence();
}
