...
class NvshmemCollectives : public Collectives {
 public:
  ...
  absl::Status DoPut(TEAMSKIND team_kind, PrimitiveType type,
                     se::DeviceMemoryBase dest, se::DeviceMemoryBase source,
                     int64_t num_elements, int pe, se::Stream& stream);

  absl::Status DoGet(TEAMSKIND team_kind, PrimitiveType type,
                     se::DeviceMemoryBase dest, se::DeviceMemoryBase source,
                     int64_t num_elements, int pe, se::Stream& stream);

  absl::Status DoQuiet(se::Stream& stream);

  absl::Status DoFence();

  absl::Status RegisterBuffer(void* addr, size_t length);

 private:
  absl::Status DoP2P(const char* op_name, PrimitiveType type, void* source_ptr,
                     void* dest_ptr, int64_t num_elements, int pe,
